...
目标服务器 45.83.106.152（现有 Caddy）。沿用 `/etc/caddy/Caddyfile` 中已有的 `/etc/caddy/sites/*.caddy` 导入，不替换其他网站配置。
...
每次上传到 `/opt/jieqi/releases/新版本/`，不要上传本地 `.env`、SQLite、node_modules 或私钥。在服务器安装两层目录的锁定依赖并构建，然后执行 `bash deploy/activate.sh /opt/jieqi/releases/新版本`。该脚本适用于现有的 Caddy 导入结构，先确保 4318/4319 没有其他服务占用。
...
检查 `systemctl status jieqi-web jieqi-api caddy`，以及三个公开域名的状态、资源与 CORS。回滚代码时将 current 指回旧 release 并重启这两个服务；数据库内容按后端发布回滚接口处理。
